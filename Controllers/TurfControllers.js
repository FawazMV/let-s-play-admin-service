...
export const reqAccept = (req, res, next) => {
    ...
    axios.put('/turf-admin/accept', { id }).then(() => res.status(200).json({ message: 'Request accepted' }))
        .catch((err) => next(err));
}
...
export const ManageTurf = (req, res, next) => {
    ...
    axios.put('/turf-admin/manage', { id, status }).then(() => res.status(200).json({ message: 'Done' }))
        .catch((err) => next(err));
}
